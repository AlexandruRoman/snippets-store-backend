...
import { templateImpressionService_isLoved } from '../service'
...
export default function templateImpressionEndpoint_isLoved() {
    const router = Router()
    router.get('/is-loved/:templateId', checkPermission(PERMISSION_LIST.MAKE_IMPRESSION), validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await templateImpressionService_isLoved({
