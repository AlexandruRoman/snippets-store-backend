...
import { templateImpressionService_count } from '../service'
...
export default function templateImpressionEndpoint_getCountsByTemplate() {
    const router = Router()
    router.get('/get-counts-by-template/:idTemplate', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await templateImpressionService_count({ idTemplate: req.params.idTemplate })
