...
import { snippetImpressionService_isLoved } from '../service'
...
export default function snippetImpressionEndpoint_isLoved() {
    const router = Router()
    router.get('/is-loved/:snippetId', checkPermission(PERMISSION_LIST.MAKE_IMPRESSION), validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await snippetImpressionService_isLoved({
