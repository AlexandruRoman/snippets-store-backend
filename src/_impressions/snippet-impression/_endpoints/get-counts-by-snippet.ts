...
import { snippetImpressionService_count } from '../service'
...
export default function snippetImpressionEndpoint_getCountsBySnippet() {
    const router = Router()
    router.get('/get-counts-by-snippet/:idSnippet', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await snippetImpressionService_count({ idSnippet: req.params.idSnippet })
