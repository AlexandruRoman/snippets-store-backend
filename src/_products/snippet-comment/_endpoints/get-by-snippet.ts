...
import { SnippetComment } from '../schema'
...
export default function snippetCommentEndpoint_getBySnippet() {
    const router = Router()
    router.get('/get-by-snippet/:idSnippet', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const snippetComments = await SnippetComment.find({ snippet: req.params.idSnippet }).populate({
            path: 'client',
            populate: { path: 'user' }
        })
        return res.status(200).json(snippetComments)
    } catch (error) {
        console.log(error)
