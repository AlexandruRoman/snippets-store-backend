...
import { templateTagDal_update } from '../dal'
...
export default function templateTagEndpoint_update() {
    const router = Router()
    router.put('/update', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const templateTag = await templateTagDal_update(req.body.id, req.body.name)
