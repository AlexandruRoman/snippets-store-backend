...
export default function templateTagEndpoint_getAll() {
    const router = Router()
    router.get('/get-all', controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const templateTags = await TemplateTag.find()
        return res.status(200).json(templateTags)
    } catch (error) {
        console.log(error)
