...
import { templateTagService_delete } from '../service'
...
export default function templateTagEndpoint_delete() {
    const router = Router()
    router.delete('/delete', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await templateTagService_delete(req.params.id)
