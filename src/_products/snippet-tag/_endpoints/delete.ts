...
import { snippetTagService_delete } from '../service'
...
export default function snippetTagEndpoint_delete() {
    const router = Router()
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await snippetTagService_delete(req.params.id)
