...
export default function snippetTagEndpoint_get() {
    const router = Router()
    router.get('/get/:id', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const snippetTag = await SnippetTag.findById(req.params.id)
        return res.status(200).json(snippetTag)
    } catch (error) {
        console.log(error)
