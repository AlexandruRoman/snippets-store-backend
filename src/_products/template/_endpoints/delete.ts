...
import { templateService_delete } from '../service'
...
export default function templateEndpoint_delete() {
    const router = Router()
    router.delete('/:id', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await templateService_delete(req.params.id)
