...
import { templateDal_getByIdPopulated } from '../dal'
...
export default function templateEndpoint_get() {
    const router = Router()
    router.get('/:id', validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const template = await templateDal_getByIdPopulated(req.params.id)
