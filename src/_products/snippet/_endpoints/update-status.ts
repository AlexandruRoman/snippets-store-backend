...
import SNIPPET_STATUS_LIST from 'src/_products/snippet-status/list'
import { snippetDal_updateStatus } from '../dal'
...
export default function snippetEndpoint_updateStatus() {
    const router = Router()
    router.put('/update-status', validation, controller)
    return router
}
...
const schema = {
    ...
    status: Joi.string()
        .allow(Object.values(SNIPPET_STATUS_LIST))
        .required()
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const snippet = await snippetDal_updateStatus(req.body)
