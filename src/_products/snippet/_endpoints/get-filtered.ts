...
import { snippetDal_getFiltered } from '../dal'
...
export default function snippetEndpoint_getFiltered() {
    const router = Router()
    router.post('/get-filtered', validation, controller)
    return router
}
...
const schema = {
    ...
    templateIds: Joi.array().items(Joi.string()),
    sellerIds: Joi.array().items(Joi.string()),
    statusIds: Joi.array().items(Joi.string()),
    page: Joi.number(),
    perPage: Joi.number()
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const snippets = await snippetDal_getFiltered(req.body)
