...
import { userService_add } from 'src/_identity/user/service'
import ROLE_LIST from 'src/_identity/role/list'
import { staffService_add } from '../service'
...
export default function staffEndpoint_add() {
    const router = Router()
...
async function controller(req: IRequest, res: Response) {
    try {
        const userModel = await staffService_add(req.body)
