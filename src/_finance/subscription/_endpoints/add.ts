...
import { subscriptionService_add } from '../service'
...
export default function subscriptionEndpoint_add() {
    const router = Router()
    router.post('/add', checkPermission(PERMISSION_LIST.VIEW_SUBSCRIPTIONS), validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const subscription = await subscriptionService_add({
