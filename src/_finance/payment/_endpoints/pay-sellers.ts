...
import { paymentService_pay } from '../service'
...
export default function paymentEndpoint_paySellers() {
    const router = Router()
    router.post('/pay-sellers', controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await paymentService_pay()
