...
import { paymentService_transfer } from '../service'
...
export default function paymentEndpoint_transferToAccount() {
    const router = Router()
    router.post('/transfer-to-account', checkPermission(PERMISSION_LIST.RECEIVE_MONEY), validation, controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await paymentService_transfer({
            iban: req.body.iban,
