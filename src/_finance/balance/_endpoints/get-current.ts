...
import { balanceService_getCurrent } from '../service'
...
export default function balanceEndpoint_getCurrent() {
    const router = Router()
    router.get('/get-current', checkPermission(PERMISSION_LIST.VIEW_BALANCE), controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const response = await balanceService_getCurrent({ idUser: req.jwtData.user.id as string })
