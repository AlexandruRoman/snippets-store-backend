...
export default function balanceEndpoint_getByUser() {
    const router = Router()
    router.get('/get-by-user', checkPermission(PERMISSION_LIST.VIEW_BALANCE), controller)
    return router
}
...
async function controller(req: IRequest, res: Response) {
    try {
        const transactions = await Balance.find({ user: req.jwtData.user.id as string })
